...
-- SELECT DISTINCT DEPT-CODE,DISTINCT JOB_CODE오류

-- DEPT_CODE,JOB_CODE 2개의 조합으로 만들어졌을때 중복제외
...
*/
--EMPLOYEE에서 부서코드가 'D9'인 사원드르이 모든컬럼 조회
SELECT*
FROM EMPLOYEE
WHERE DEPT_CODE='D9';

--EMPLOYEE에서 부서코드가 'D1'이 아닌 사원들의 사번, 성명, 부서코드 조회
SELECT EMP_ID,EMP_NAME,DEPT_CODE
FROM EMPLOYEE 
WHERE DEPT_CODE <> 'D1'

--EMPLOYEE에서 급여가 400만원 이상인 사원드릐 사원명,부서코드,급여 조회
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE SALARY >= 4000000;

--EMPLOYEE에서 재직중인 사원의 사번,사원몀,입사일 조회
...
--급여가 350만원 이상 600만원 이하인 사원울 제외한 사번, 사원명,급여 조회
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE
WHERE NOT SALARY BETWEEN 3500000 AND 6000000;

--입사일이 90/01/01~99/12/31 사이인 사원의 사번, 사원명, 입사일 조회
SLECT EMP_ID,EMP_NAME,_HIRE_DATE
FROM EMPLOYEE
WHERE BETWEEN '90/01/01' AND '01/12/31;
...
%:0글자이상
EX)비교대상컬럼 LIKE'문자%' => 비교대상 컬럼 값이 '문자로' 시작되는 것들을 조회
   비교대상컬럼 LIKE'%문자' => 비교대상 컬럼 값이 '문자로' 끝나는 것들을 조회
   비교대상컬럼 LIKE'%문자%' = >  비교대상 컬럼 값이 '문자'가 포함되는 것들은 모두 조회
   
   *'_':1글자
   EX)비교대상 컬럼 LIKE'_문자' = > 비교대상 컬럼값의 '문자'앞에 무조건 한글자가 올 경우 조회
 EX)비교대상 컬럼 LIKE'문자_' = > 비교대상 컬럼값의 '문자'뒤에 무조건 한글자가 올 경우 조회
*/
...
--사원들 중 가운데 '하'가 들어있는 사원들의사번, 사원명 조회
...
SELECT EMP_ID,EMP_NAME,EMAIL
FROM EMPLOYEE
WHERE NOT EMAIL LIKE'___e%'ESCAPE'e';

--------------------------------------실습문제-------------------
--1. 이름이 '연'으로 끝나는 사원의 사번, 사원명, 입사일조회
SELECT EMP_ID,EMP_NAME,HIRE_DATE
FROM EMPLOYEE
...
--3. 이름이 '하'가 포함되어 있고, 급여가 250만원 이상인 사원들의 사원명,급여조회
...
>>IS NULL/IS NOT NULL
컬럼값에 NULL이 있는 경우NULL값 비교에 사용되는 연산자
...
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
--WHERE DEPT_CODE='D5' OR DEPT_CODE='D6' OR DEPT_CODE='D8'; 을 짧게 사용할수있음
WHERE DEPT_CODE IN('D5','D6','D8');

--부서코드가 D5이거나 D6이거나 D8이 아닌 사원명, 부서큳, 급여조회
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
...
--2. 연봉(보너스포함 X)이 3000만원 이상이고 보너스를 받지 않은 사언들의 사번, 사원명, 보너스,연봉 조회
...
FROM EMPLOYEE
WHERE HIRE_DATE >= '95/01/01' AND DEPT_CODE IS NOT NULL; -- 날짜에 ''붙이기
...
--5. 보너스포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어있는 사원들의 사번, 사원명, 급여,보너스포함 연봉 조회(별칭부여)
SELECT EMP_ID,EMP_NAME,SALARY,SALARY*(1+BONUS)*12
FROM EMPLOYEE
...
/*
   <ORDER BY 절>
   정렬
   -SELECT 문 가장 마지막 줄에 작성, 실행순서 또한 맨마지막에 실행
   ...
   *NULLS FIRST : 정렬하고자하는 컬럼값에 NULL 이 있는경우 해당 데이터를 맨 앞에 배치(생략시 DESC일때는 기본값)
      *NULLS FIRST : 정렬하고자하는 컬럼값에 NULL 이 있는경우 해당 데이터를 맨 뒤에 배치(생략시 ASC일때의 기본값)

*/
--보너스로정렬
SELECT EMP_NAME,BONUS,SALARY
FROM EMPLOYEE
--ORDER BY BONUS; --오름차순 정렬일때 NULL 제일 뒤 위치
--ORDER BY BONUS NULLS FIRST; -- 오름차순인데 NULL 을 위로 하고싶다
--ORDER BY BONUS DESC; -- 내림차순은 반드시 DESC를 기술, NULL은 맨 앞에 옴
