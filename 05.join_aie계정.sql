...
JOIN은 크기"오라클 전용구문"과 "ANIS"구문 (ANSI=미국국립표준협회)
            [용어정리]
            오라클 정용 구문     |       ANSI구문
            --------------------------------------
                ...
                --------------------------------------------------------------
                포괄조인         | 외부 내부조인(INNER JOIN)=>JOIN USING/ON
...
/* 1.등가조인(EQUAL JOIN/내부조인(INNER JOIN)
     연결시키고자하는 컬럼 값이 "일치하는 행"들만 조인되어 조회(=일치하는 값이 없으면 조회에서 제외)
     */
     --오라클 전용 구문
     /*
     FROM에 조회하고자하는 테이블들을 나열(, 구분자로)
     WHERE절에 매칭시킬 컬럼(연결고리)에 조건 제시함
     */
...
JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID);
     ...
     --2. 연결할 두컬럼명이 같은 경우(EMPLOYEE:JOB_CODE,JOB:JOB_CODE)
     --JOIN ON,JOIN USING모두 사용 가능
     --전체 사원들의 사번,사원명,직급코드, 직급명을 조회
     SELECT EMP_ID,EMP_NAME,JOB_CODE,JOB_NAME
     FROM EMPLOYEE
     JOIN JOB ON(JOB_CODE,JOB_CODE);   --오류
     
     --해결방법1. 테이블에 별칭을 이용하는방법
     SELECT EMP_ID,EMP_NAME,E.JOB_CODE,JOB_NAME
     FROM EMPLOYEE E
     JOIN JOB J ON(E.JOB_CODE=J.JOB_CODE);
     
     --해결방법2. JOIN USING구문을 사용하는 방법(두 컬럼명이 일치할때만 사용가능)
     SELECT EMP_ID,EMP_NAME,JOB_CODE,JOB_NAME
     FROM EMPLOYEE
     ...
     --2.DEPARTMENT와 LOCATRION을 참고하여 전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회
     --> 오라클 구문 전용
     ...
     */
     ...
     --1)LEFT[OUTER] JOIN : 두 테이블중 왼쪽에 기술된 테이블이 기준으로 JOIN
     -->ANSI 구문
     SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
     FROM EMPLOYEE
     ...
     SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
     FROM EMPLOYEE,DEPARTMENT
     SHERE DEPT_CODE=DEPT_ID(+); --기준이 아닌 테이블의컬럼명 뒤에(+)기호를 붙임
     
     
     --1)RIGHT[OUTER]JOIN : 두 테이블중 오른쪽에 기술된 테이블이 기준으로 JOIN
     --> ANSI구문
     SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
     FROM EMPLOYEE
    ...
    --3.FULL[OUTER] JOIN : 두 테이블에 기술된 모든행을 조회(단, 오라클 전용 구문 없음)
     SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
     FROM EMPLOYEE
     FULL JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID);
     ---------------------------------------------------------------------------
     /*3.비등가 조인(NON EQUAL JOIN)
     매칭시킬 컬럼에 대한 조건 작성시 '='를 사용하지 않는 JOIN문
     ...
     */
     ...
     /*               가져올 컬럼명    JOIN될 컬럼명
     EMPLOYEE=>EMP_ID,EMP_NAME DEPT_CODE    JOB CODE
     DEPARTMENT=>    DEPT_TITLE, DEPT_ID
     JOB = >      JOB_NAME                  JOB CODE
     */
     --=> 오라클 구문
     ...
     ----------------------------------------------------실습문제-----------------
     --사번,사원명,부서명,지역명,국가명 조회(EMPLOYEE,DEPARTMENT,LOCATION,NATIONAL조인)
     --> 오라클 구문
     SELECT EMP_ID,EMP_NAME,DEPT_TITLE,NATIONAL_NAME
     FROM EMPLOYEE E,DEPARTMENT D,LOCATION L,NATIONAL N
     WHERE DEPT_CODE=DEPT_ID
     AND LOCATION_ID=LOCAL_CODE
     AND L.NATIONAL_CODE=N.NATIOANL_CODE;
     ...
     --사번, 사원명, 부서명,직급명, 지역명, 국가명, 급여등급 조회(모든테이블 다 조인)
     --=> 오라클 
     SELECT EMP_ID,EMP_NAME,DEPT_TITLE,JOB_NAME,LOCAL_NAME,NATIONAL_NAME,SAL_LEVEL
     FROM EMPLOYEE E,DEPARTMENT D,JOB J,LOCATION N,SAL_GRADE S
     WHERE DEPT_CODE=DEPT_ID
     AND E.JOB_CODE=J.JOB_CODE
     AND LOCATION_ID = LOCAL_CODE
     ...
     JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
     JOIN JOB USING(JOB_CODE)
     JOIN LOCATION ON(LOCATIOON_ID=LOCAL_CODE)
     JOIN NATIONAL ON USING(NATIONAL_CODE)
     JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL);
