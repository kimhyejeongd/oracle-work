   ...
   *트랜잭션
   ...
   -데이터의 변경사항(DML)들을 하나의 트랜젝션에 묶어서 처리
    DML문 한개를 수행할 때 트랜잭션이 존재하면 해당 트랜잭션에 같이 묶어서 처리
      ...
      -트랜잭션의 대상이 되는 SQL : INSERT,UPDATE,DEDLETE
      ...
      -SAVEPOINT 포인트명; 진행 : 현재 이 시점에 해당 포인명으로 임시저장임을 정의해두는 것
               ROLLBACK진행시 전체 변경사항들을 다 삭제하는게 아니라 일부만 롤백 가능
...
--501번 추가
INSERT INTO EMP_01 VALUES(501,'이세종',2000000, 0.1);
 ...
 */
 ...
 --DDL 구문을 실행하는 순간 COMMIT 이 됨
 
 ROLLBACK;
 -->DDL 문(CREATE,ALTER,DROP)을 수행하는 순간 트랜잭션이 있던 변경사항들은 무조건 COMMIT 이 됨
