...
EXCEPTION
  WHEN TOO_MANY_ROWWS THEN DBMS_OUTPUT.PUT_LINE('조회된 행이 너무 많습니다');
  WHEN NO_DATE_FROUNC THEN DBMS_OUTPUT.PUT_LINE('조회된 행이 없습니다');
  END;
  /
-----------------------------------연습문제
/* 사원의 연봉을 구하는 PL/SQL 작성,보너스가 있는 사원은 보너스도 포함하여 계산
2.구구단 짝수단 출력
...
BEGIN
  FOR DAN IN 2..9
  LOOP
  IF MOD(DAN,2)=0  --2로 나누었을때 나머지가 0
    THEN 
      FOR SU IN 1..9
      LOOP
--      RESULT := DAN*SU;
