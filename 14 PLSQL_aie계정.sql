/*
  ...
  *PL/SQL 구조
   -[선언부(DECLARE SECCTION)] : DECLARE로 시작,변수나 상수를 선언 및 초기화하는 부분
   - 실행부(EXECUTABLE SECTION) : BEGIN으로 시작,SQL문 또는 제어문(조건문,반복문)등의 로직을 기술하
   -[예외처리부 (EXCEPTION SECTION)] : EXCEPTION으로 시작,
          예외 발생시 해결하기 위한 구문을 미리 기술해 둘 수 있는 부분
          
          */
-- * 화면에 오라클 출력(껐다 다시 켜면 반드시 실행해야함)
SET SERVEROUTPUT ON;
...
DECLARE
  ...
  SELECT EMP_ID,EMP_NAME,SALARY
   ...
   ----------------------실습문제------------------------------------------------
   /*
     레퍼런스타입변수로 EID,ENAME,JCODE,SAL,DITLE을 선언하ㅏ고
